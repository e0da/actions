...
require_literal "$workflow" "        default: 6000.3.22f1"
...
require_literal "$workflow" "TEST_RESULTS_DIR: \${{ inputs['test-results-dir'] }}"
require_literal "$workflow" "unsafe test-results-dir"
require_literal "$workflow" "unsafe test-platform"
...
require_literal "$workflow" "test_results_file=\"\$TEST_RESULTS_DIR/\${TEST_PLATFORM}.xml\""
require_literal "$workflow" "rm -f \"\$test_results_file\""
require_literal "$workflow" "-testResults \"\$test_results_file\""
require_literal "$workflow" "if [ ! -s \"\$test_results_file\" ]; then"
require_literal "$workflow" 'Unity Test Runner did not create a non-empty results file'

test_step=$(sed -n '/^      - name: Run Unity tests$/,/^      - name: Build Android player$/p' "$workflow")
if printf '%s\n' "$test_step" | grep -F -- "-quit" >/dev/null; then
  fail "$workflow Run Unity tests step must not pass -quit"
fi
...
require_literal "$workflow" "\${{ runner.temp }}/unity-logs"
require_literal "$workflow" "\${{ inputs.test-results-dir }}"
require_literal "$readme" "defaults to Unity \`6000.3.22f1\`"
require_literal "$readme" "requires a non-empty Unity Test Runner result XML"
